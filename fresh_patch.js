...
if(!s.includes('/* FRESH_RULES_V4 */')){
const freshFn=`/* FRESH_RULES_V4 */
function fresh(rows){
  const r0=rows[0]||{};
  ...
  const classify=(r,p)=>{const c=norm(kcat?r[kcat]:''); const n=norm(p);
    if(/bia|sua|thuc uong|nuoc giai khat|nuoc ngot|sua chua|dong lanh|kem/.test(c+' '+n)) return null;
...
const sectionFn=`function freshSection(rows){
  ...
  const v=(n)=>new Intl.NumberFormat('vi-VN').format(Math.round(n||0));
  const money=(n)=>v(n)+' đ';
  const esc2=x=>String(x??'').replace(/[&<>\"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','\\':'&#92;','"':'&quot;'}[m]));
  const render=(g)=>{const d=data.filter(x=>x.group===g);const total=d.reduce((s,x)=>s+x.doanh_thu,0);const good=d.slice().sort((a,b)=>b.doanh_thu-a.doanh_thu).slice(0,5);const bad=d.slice().sort((a,b)=>((b.mat_mat+b.huy)-(a.mat_mat+a.huy))||((a.sl_ban||0)-(b.sl_ban||0))||(b.giam_gia_pct-a.giam_gia_pct)).slice(0,5);
    const goodRows=good.map(x=>'<tr><td>'+esc2(x.san_pham)+'</td><td>'+money(x.doanh_thu)+'</td><td>'+v(x.sl_ban)+'</td><td>'+x.nguyen_gia_pct.toFixed(1)+'%</td><td>'+x.giam_gia_pct.toFixed(1)+'%</td><td>Giữ tồn, ưu tiên trưng bày & duy trì nhập</td></tr>').join('');
    const badRows=bad.map(x=>'<tr><td>'+esc2(x.san_pham)+'</td><td>'+v(x.mat_mat)+'</td><td>'+v(x.huy)+'</td><td>'+x.giam_gia_pct.toFixed(1)+'%</td><td>Giảm nhập; xử lý tồn; rà soát giá & trưng bày</td></tr>').join('');
    return '<div class="sub"><h3>'+groupTitle[g]+' <span>DT '+money(total)+'</span></h3><div class="tw"><div><b>🟢 5 SKU bán tốt</b><table><tr><th>SP</th><th>DT</th><th>SL</th><th>NL</th><th>GG</th><th>🎯</th></tr>'+goodRows+'</table></div><div><b>🔴 5 SKU bán kém</b><table><tr><th>SP</th><th>SL MẤT MÁT</th><th>SL HỦY</th><th>GG</th><th>🎯</th></tr>'+badRows+'</table></div></div></div>';};
  const total=data.reduce((s,x)=>s+x.doanh_thu,0);return '<!--FRESH_REPORT_START--><div id="fresh-report" class="section compact-fresh"><h2>🥬 FRESH — 01–12/08</h2><div class="muted">SL bán đúng từ ngày 01 đến ngày hiện tại · Bán tốt: DT cao · Bán kém: bán ít + mất mát/hủy cao + % giảm giá lớn · Bia/Sữa loại khỏi FRESH.</div>'+groups.map(render).join('')+'<div class="muted" style="font-weight:900">TỔNG FRESH: '+money(total)+'</div></div><!--FRESH_REPORT_END-->';
}
`;
