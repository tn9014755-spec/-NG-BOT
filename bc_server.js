...
async function reportMessages(report,withImage=true){const out=[];if(withImage){const img=await imageMessage(report);if(img)out.push(img)}out.push(...chunks(report).map(text=>({type:'text',text})));return out.slice(0,5)}
