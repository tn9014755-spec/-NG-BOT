...
const {downloadTemplate,applyTemplate,putJson,putLocalJson,readLocalJson,latestLocalJson,readJson,latestJson,listJson,hasDriveCredentials}=require("./bc_drive");
const {buildBCFlex}=require("./bc-flex");
const fresh=require("./fresh_report");
...
function latestReportContext(){
 const parts=[];
 try{
  if(typeof fresh.aiHistoryContext==="function"){
   const h=fresh.aiHistoryContext(8);
   if(h)parts.push("LỊCH SỬ BÁO CÁO FRESH & ĐÔNG MÁT (MỚI NHẤT ĐẾN CŨ HƠN):\n"+h);
  }else if(fresh.exists()&&typeof fresh.aiContext==="function"){
   const x=fresh.aiContext();if(x)parts.push("DỮ LIỆU FRESH & ĐÔNG MÁT MỚI NHẤT:\n"+x)
  }
 }catch(e){console.warn("⚠️ Không đọc được lịch sử DATA FRESH cho AI:",e.message)}
...
app.get("/fresh.html",(_,res)=>{if(!fresh.exists())return res.status(404).send("Chưa có báo cáo FRESH.");res.type("html").send(fresh.page())});
