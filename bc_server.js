...
async function reply(token,messages){if(!TOKEN){console.error('LINE TOKEN MISSING');return false}if(!token){console.error('REPLY TOKEN MISSING');return false}const ms=Array.isArray(messages)?messages:[messages];const r=await line(API+'/message/reply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({replyToken:token,messages:ms.slice(0,5)})});console.log('LINE REPLY',r.status);if(!r.ok)console.error(await r.text());return r.ok}
...
app.get('/health',(_,res)=>res.json({ok:true,bot:'BC_NGAY',token:!!TOKEN,secret:!!SECRET,target:!!TARGET,data:bcNgay.hasData(),memory:!!LAST_REPORT}));
...
app.post('/webhook',async(req,res)=>{const b=req.rawBody||Buffer.alloc(0);const sig=req.get('x-line-signature')||'';console.log('WEBHOOK RECEIVED bytes='+b.length+' signature='+(sig?'yes':'no'));if(SECRET){const h=crypto.createHmac('sha256',SECRET).update(b).digest('base64');if(!sig||sig.length!==h.length||!crypto.timingSafeEqual(Buffer.from(sig),Buffer.from(h))){console.error('WEBHOOK SIGNATURE INVALID');return res.status(401).send('invalid signature')}}res.status(200).send('OK');let p;try{p=JSON.parse(b.toString('utf8'))}catch(e){console.error('WEBHOOK JSON ERROR',e);return}console.log('WEBHOOK EVENTS',p.events?p.events.length:0);for(const e of p.events||[]){console.log('LINE EVENT',e.type,e.source&&e.source.type);if(!e.replyToken||e.mode==='standby')continue;const m=e.message||{};
if(m.type==='text'){const t=String(m.text||'').trim().toUpperCase().replace(/\s+/g,' ');console.log('LINE TEXT',t);if(t==='ID'||t==='GROUP ID'||t==='GROUPID'){const s=e.source||{};const id=s.groupId||s.roomId||s.userId||'(không xác định)';const kind=s.groupId?'GROUP':s.roomId?'ROOM':'USER';await reply(e.replyToken,{type:'text',text:'🆔 ID NHÓM\n'+id+'\n\n📌 Loại: '+kind+'\n📋 Copy ID này gửi lại cho em.'});continue}if(t==='BC NGÀY'||t==='BC NGAY'){const report=LAST_REPORT||(bcNgay.hasData()?bcNgay.report():'');if(!report)await reply(e.replyToken,{type:'text',text:'📭 Chưa có data BC NGÀY. Anh gửi file Excel trước nhé.'});else await reply(e.replyToken,reportMessages(report));continue}}
if(m.type==='file'){console.log('LINE FILE',m.fileName||'(no name)',m.id||'(no id)');const name=String(m.fileName||'').toLowerCase();if(!/\.(xlsx|xls|csv)$/.test(name)){await reply(e.replyToken,{type:'text',text:'⚠️ Chỉ nhận file Excel/CSV.'});continue}try{const r=await fetch(DATA_API+'/message/'+encodeURIComponent(m.id)+'/content',{headers:{Authorization:'Bearer '+TOKEN}});if(!r.ok)throw new Error('LINE download '+r.status+' '+await r.text());const buf=Buffer.from(await r.arrayBuffer());bcNgay.saveUpload(buf,m.fileName||('BC_NGAY_'+Date.now()+'.xlsx'));LAST_REPORT=bcNgay.report();console.log('BC NGAY FILE SAVED OK',m.fileName,'report_ready=true');await reply(e.replyToken,[{type:'text',text:'✅ Đã nhận file BC NGÀY và xử lý xong.'},...reportMessages(LAST_REPORT)]);await schedule()}catch(err){console.error('FILE ERROR',err);await reply(e.replyToken,{type:'text',text:'❌ File chưa hợp lệ hoặc bot chưa đọc được file. Data cũ vẫn được giữ nguyên.'}).catch(()=>{})}}}});
