...
function add(text,target,userId){const m=parseMeeting(text);if(!m)return null;const items=load().filter(x=>x.target!==target||x.when!==m.when);const item={id:Date.now().toString(36),...m,target,userId:userId||'',reminded:false,createdAt:new Date().toISOString()};items.push(item);save(items);return item}
