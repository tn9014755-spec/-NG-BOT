...
function parse(buf){
 ...
 for(const r of rows){
  ...
  const sale=num(find(r,["doanh thu"]));
...
function save(buf,file){const rows=parse(buf);if(!rows.length)throw Error("Không nhận diện được dữ liệu FRESH/ĐÔNG MÁT trong file");const html=build(rows,file);fs.writeFileSync(XLS,buf);fs.writeFileSync(HTML,html);const sale=sum(rows,"sale"),loss=sum(rows,"loss"),meta={fileName:file,updatedAt:new Date().toISOString(),sale,loss,pct:pct(loss,sale),rows:rows.length};fs.writeFileSync(META,JSON.stringify(meta,null,2));return meta}
