...
const XLS=path.join(DIR,"latest.xlsx"),HTML=path.join(DIR,"latest.html"),META=path.join(DIR,"latest.json"),PENDING=path.join(DIR,"pending.json"),HISTORY=path.join(DIR,"history");fs.mkdirSync(HISTORY,{recursive:true});
...
function save(buf,file){
 const rows=parse(buf);if(!rows.length)throw Error("Không nhận diện được dữ liệu FRESH/ĐÔNG MÁT trong file");
 const html=build(rows,file),products=new Map();
 for(const r of rows){const key=[r.c,r.code||r.name,r.name].join("|"),p=products.get(key)||{sale:0,sold:0,mm:0,ncc:0,ht:0};p.sale+=Number(r.sale||0);p.sold+=Number(r.sold||0);p.mm+=Number(r.mm||0);p.ncc+=Number(r.ncc||0);p.ht+=Number(r.ht||0);products.set(key,p)}
 let sale=0,loss=0;for(const p of products.values()){sale+=p.sale;const unit=p.sold?p.sale/p.sold:0;loss+=(p.mm+p.ncc+p.ht)*unit}
 const now=new Date(),id=now.toISOString().replace(/[:.]/g,"-"),meta={id,fileName:file,updatedAt:now.toISOString(),sale,loss,pct:pct(loss,sale),rows:rows.length};
 fs.writeFileSync(XLS,buf);fs.writeFileSync(HTML,html);fs.writeFileSync(META,JSON.stringify(meta,null,2));
 const snapshot={...meta,data:aiData(rows,meta)};fs.writeFileSync(path.join(HISTORY,"fresh-"+id+".json"),JSON.stringify(snapshot,null,2));return meta
}
...
function aiData(rows,meta={}){
 const products=new Map();
 for(const r of rows){const key=[r.c,r.code||r.name,r.name].join("|"),p=products.get(key)||{c:r.c,name:r.name,code:r.code,sale:0,sold:0,mm:0,ncc:0,ht:0};p.sale+=Number(r.sale||0);p.sold+=Number(r.sold||0);p.mm+=Number(r.mm||0);p.ncc+=Number(r.ncc||0);p.ht+=Number(r.ht||0);products.set(key,p)}
 const a=[...products.values()].map(p=>({...p,loss:(p.mm+p.ncc+p.ht)*(p.sold?p.sale/p.sold:0)})),sale=sum(a,"sale"),loss=sum(a,"loss");
 const groups=["FRESH","ĐÔNG MÁT"].map(g=>{const z=a.filter(x=>bucket(x.c)===g);return{nhom:g,doanhThu:sum(z,"sale"),haoHut:sum(z,"loss"),tyLe:pct(sum(z,"loss"),sum(z,"sale")),nganh:[...new Set(z.map(x=>x.c))].map(c=>{const y=z.filter(x=>x.c===c);return{ten:c,doanhThu:sum(y,"sale"),haoHut:sum(y,"loss"),tyLe:pct(sum(y,"loss"),sum(y,"sale"))}}).sort((x,y)=>y.haoHut-x.haoHut)}});
 ...
 return {loaiBaoCao:"FRESH & ĐÔNG MÁT",fileName:meta.fileName||"",updatedAt:meta.updatedAt||"",soDong:rows.length,tongDoanhThu:sale,tongHaoHut:loss,tyLe:pct(loss,sale),nhom:groups,topMatHang:top}
}
