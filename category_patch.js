...
const fg=`function freshGroup(row,kp,kc){const p=String(kp?row[kp]:'').trim();const c=String(kc?row[kc]:'').trim();if(!p)return null;const all=(c+' '+p).toLowerCase();if(/c\\.p\\b|c\\s*p\\b/.test(all))return null;const cat=c.toLowerCase();if(cat){if(/bia|sữa|sua|nước|nuoc|kem|trứng|trung|bánh|banh|đông lạnh|dong lanh|hàng mát|hang mat|thức uống|thuc uong|đồ uống|do uong|chăm sóc|cham soc|hóa mỹ phẩm|hoa my pham/.test(cat))return null;if(/rau\\s*củ|rau\\s*cu|rau\\s*củ\\s*quả|rau\\s*cu\\s*qua|rau củ quả cl|rau cu qua cl/.test(cat))return'RAU CỦ';if(/thịt|thit/.test(cat))return'THỊT';if(/cá|ca|hải sản|hai san/.test(cat))return'CÁ / THỦY HẢI SẢN';if(/trái cây|trai cay/.test(cat))return'TRÁI CÂY';return null}const sp=p.toLowerCase();if(/bia|sữa|sua|nước|nuoc|kem|trứng|trung|bánh|banh|đông lạnh|dong lanh|hàng mát|hang mat|thức uống|thuc uong/.test(sp))return null;if(/thịt|thit|heo|bò|bo|gà|ga|sườn|suon|cánh gà|canh ga|đùi|dui|xương|xuong|chân giò|chan gio/.test(sp))return'THỊT';if(/cá|\\bca\\b|hải sản|hai san|tôm|tom|mực|muc|cua|ghẹ|ghe|nghêu|ngheu|sò|so /.test(sp))return'CÁ / THỦY HẢI SẢN';if(/trái cây|trai cay|dưa hấu|dua hau|táo|tao |nho |xoài|xoai|chuối|chuoi|thanh long|dưa lưới|dua luoi|kiwi|lê |le |cam |quýt|quyt|bưởi|buoi|mận|man |đào|dao |dâu|dau /.test(sp))return'TRÁI CÂY';if(/rau|củ|cu |nấm|nam |bắp cải|bap cai|cải|cai |cà rốt|ca rot|hành|hanh |khoai|bắp mỹ|bap my|dưa leo|dua leo/.test(sp))return'RAU CỦ';return null}`;
