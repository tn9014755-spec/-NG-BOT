...
// Hard-first webhook gate. This is intentionally outside the old FRESH installer
// so group commands cannot fall through to BC NGAY.
      ...
      const hardGate=async(req,res,next)=>{
        try{
          ...
          for(const e of p.events||[]){
            ...
            if(m.type==='text'&&/^BC\\s+FRESH$/i.test(String(m.text||'').trim())){
              handled=true; if(s.type==='group'||s.type==='room')setTarget(chat); globalThis.__BC_FRESH_WAITING=true;
              await reply(e.replyToken,{type:'text',text:'✅ Đã nhận lệnh BC FRESH.\\n📎 Anh gửi FILE EXCEL tiếp theo, em sẽ trả 1 ảnh FULL báo cáo.'}); continue;
            }
            if(m.type==='text'&&/^FRESH$/i.test(String(m.text||'').trim())){
              handled=true; globalThis.__BC_FRESH_WAITING=false;
              if(fs.existsSync(IMG))await reply(e.replyToken,{type:'image',originalContentUrl:BASE+'/bc-fresh.png?'+Date.now(),previewImageUrl:BASE+'/bc-fresh-preview.png?'+Date.now()});
              else await reply(e.replyToken,{type:'text',text:'⚠️ Chưa có BC FRESH gần nhất. Anh gửi BC FRESH + file Excel trước nhé.'}); continue;
            }
            if(m.type==='file'&&/\\.(xlsx|xls|csv)$/i.test(String(m.fileName||''))&&globalThis.__BC_FRESH_WAITING===true){
              handled=true; await reply(e.replyToken,{type:'text',text:'⏳ Đã nhận file Excel. Em đang dựng ảnh FULL BC FRESH...'});
              try{const r=await api(DATA_API+'/message/'+encodeURIComponent(m.id)+'/content');if(!r.ok)throw Error('LINE tải file lỗi '+r.status);const im=await globalThis.__BC_FRESH_PROCESS(Buffer.from(await r.arrayBuffer()),m.fileName||'latest.xlsx');globalThis.__BC_FRESH_WAITING=false;const dest=getTarget()||chat;if(dest)await push(dest,{type:'image',originalContentUrl:im.full+'?'+Date.now(),previewImageUrl:im.preview+'?'+Date.now()});}
              catch(err){globalThis.__BC_FRESH_WAITING=false;console.error('BC FRESH HARD GATE',err);const dest=getTarget()||chat;if(dest)await push(dest,{type:'text',text:'❌ BC FRESH lỗi: '+String(err.message||err).slice(0,220)});} continue;
            }
          }
